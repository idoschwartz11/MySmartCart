...
async function main() {
  ...
  // מפרקים רק PriceFull שהורדו ולא פורקו
  ...
  for (const f of files) {
    ...
    if (dl.error) {
      await supabase.from("raw_files").update({ status: "failed", error: dl.error.message }).eq("id", f.id);
    ...
    const xmlText = new TextDecoder("utf-8").decode(pako.ungzip(new Uint8Array(ab)));
    ...
    const storeIdNorm = storeIdStr
      ? storeIdStr.padStart(3, "0")
      : (f.store_id ? String(f.store_id).padStart(3, "0") : null);
    ...
    for (const it of items) {
      ...
      const itemCode = it.ItemCode != null ? String(it.ItemCode) : "";
      if (!itemCode) continue; // חובה בשביל unique(raw_file_id,item_code)

      const barcode = /^\d{8,14}$/.test(itemCode) ? itemCode : null;
    ...
    await supabase.from("raw_files").update({ status: "parsed", error: null, store_id: storeIdNorm }).eq("id", f.id);
